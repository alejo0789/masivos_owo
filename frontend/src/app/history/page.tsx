...
import { getHistory, getStats, getHistoryCount, deleteHistory, exportHistory, MessageLog, Stats } from '@/lib/api';
...
export default function HistoryPage() {
    ...
    const handleDeleteHistory = async () => {
        // Validation: Must have at least one filter
        if (!dateFrom && !dateTo && !channel && !status && !search) {
            alert('Por favor seleccione al menos un filtro (como un rango de fechas) para realizar la limpieza del historial.');
            return;
        }

        const confirmMsg = `¿Está seguro de que desea borrar los registros que coinciden con los filtros seleccionados? Esta acción no se puede deshacer.`;
        ...
        try {
            ...
            const result = await deleteHistory(params);
            alert(`${result.message}. Se eliminaron ${result.deleted_count} registros.`);
            ...
            alert('Error al limpiar el historial: ' + detail);
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleExportHistory = async () => {
        try {
            ...
            alert('Error al exportar el historial a formato Excel.');
        } finally {
            setLoading(false);
        }
    };
                ...
                <div className="p-5 flex flex-col gap-5">
                    ...
                    <div className="flex flex-wrap gap-4 items-center border-t border-gray-100 pt-5">
                        ...
                        <button
                            ...
                            className="btn bg-green-50 text-green-600 border-green-100 hover:bg-green-600 hover:text-white transition-colors py-2 flex items-center gap-2"
                            disabled={loading}
                        ...
                        <button
                            ...
                            className="btn bg-red-50 text-red-600 border-red-100 hover:bg-red-600 hover:text-white transition-colors py-2 flex items-center gap-2"
                            disabled={loading}
                            title="Borrar registros según filtros"
                        >
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                                <path d="M3 6h18" />
